                    ...
                    <div class="card mx-auto shadow-lg" style="width: 12rem; min-height: 400px;">
                            <img src="{{Voyager::image($category->image)}}" class="card-img-top" alt="{{$category->name}}">
                                ...
                                <div class="mx-auto">
                                    ...
                                    <span class="badge badge-danger badge ml-2 mr-2 p-1 font-weight-lighter" >
                            ...
                            <div class="card-body text-justify ml-1">
                                <div class="p-2" style="overflow:hidden; color:#73726c;">{{ \Illuminate\Support\Str::limit(strip_tags($category->description), 100, $end='...') }}</div>
              ...
              @foreach($packages as $category)
                    ...
                    <img src="{{Voyager::image($category->image)}}" class="card-img-top" alt="{{$category->name }}">
                        ...
                        <span class="card-text p-2 font-weight-bold text-uppercase" style="color:#3c3c3c;"> {{$category->name }}</span>
                         <h5><span class="display-6 font-weight-bolder text-info"><img src="https://img.icons8.com/color/64/000000/rupee.png" width="32" height="32"/>{{ $category->amount }}</span> <strike class="font-weight-lighter">{{$category->amount + rand(2,100)}}</strike> </h5>
                         <div class="mx-auto">
                         <span class="tnlite-badge tnlite-heading-xs badge-success p-1 font-weight-lighter">{{ $category->board }}</span>
                            <span class="tnlite-badge tnlite-heading-xs ml-2 mr-2 badge-danger p-1 font-weight-lighter" > {{ $category->class }}th class</span>
                            <span class="tnlite-badge tnlite-heading-xs badge-info p-1 font-weight-lighter">{{ $category->subject }}</span>
                            </div>
                    <div class="card-body text-justify ml-1">
                        <div class="p-2" style="overflow:hidden; color:#73726c;">{{ \Illuminate\Support\Str::limit(strip_tags($category->description), 100, $end='...') }}</div>
                    </div>
