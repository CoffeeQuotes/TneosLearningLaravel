              ...
              @foreach($packages as $category)
                    ...
                    <img src="{{Voyager::image($category->image)}}" class="card-img-top" alt="{{$category->name }}">
                        ...
                        <span class="card-text p-2 font-weight-bold text-uppercase" style="color:#3c3c3c;"> {{$category->name }}</span>
                         <h5><span class="display-6 font-weight-bolder text-info"><img src="https://img.icons8.com/color/64/000000/rupee.png" width="32" height="32"/><strike>{{$category->amount + str_random(100)}}</strike> {{ $category->amount }}  /- only</span></h5>
                         <div class="mx-auto">
